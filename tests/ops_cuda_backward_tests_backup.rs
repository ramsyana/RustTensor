...
fn test_cuda_exp_backward() -> Result<(), Error> {
    ...
    // dx = dout * exp(x)
    let n = y.size() as f32;
    let expected_grad = vec![1.0 / n, std::f32::consts::E / n]; // e constant from std
...
// Add a consistent helper for CUDA initialization
fn init_cuda_for_test(test_name: &str) -> Result<CudaContextGuard, Error> {
    println!("[{}] Initializing CUDA context...", test_name);
    init_context(0)
        .map_err(|e| Error::CudaError(format!("{} - Context init failed: {}", test_name, e)))?;
    CudaContextGuard::new()
        .map_err(|e| Error::CudaError(format!("{} - Guard creation failed: {}", test_name, e)))
}

// Add a helper to print tensors for debugging
fn debug_print_tensor<B: Backend>(test_name: &str, name: &str, tensor: &B::Storage) {
    if let Ok(data) = B::copy_to_host(tensor) {
        ...
        println!(
            "[{}] Failed to copy {} to host for debugging",
            test_name, name
        );
    }
}
...
#[test]
fn test_cuda_min_backward() -> Result<(), Error> {
    let test_name = "test_cuda_min_backward";
    let _guard = init_cuda_for_test(test_name)?;
    println!("[{}] CUDA context initialized", test_name);

    let data = vec![1.0, 5.0, 2.0, 3.0, 1.0, 6.0];
    let shape = &[2, 3];

    // Create Tensor objects
    let x_cpu_storage = CpuBackend::from_vec(data.clone(), shape)?;
    let x_cuda_storage = CudaBackend::from_vec(data, shape)?;
    let x_cpu = Tensor::<CpuBackend>::new(x_cpu_storage, false);
    let x_cuda = Tensor::<CudaBackend>::new(x_cuda_storage, false);

    // Gradient for global reduction - use empty shape for scalar
    let grad_output_cpu = CpuBackend::ones(&[])?;
    let grad_output_cuda = CudaBackend::ones(&[])?;

    println!("[{}] Testing global reduction", test_name);
    // Test global min backward
    let op_cpu = Op {
        op_type: OpType::Min(None),
        inputs: vec![x_cpu.clone()],
        backward_fn: Rc::new(|_, _| panic!("Should not be called")),
        cached_outputs: None,
    };
    let op_cuda = Op {
        op_type: OpType::Min(None),
        inputs: vec![x_cuda.clone()],
        backward_fn: Rc::new(|_, _| panic!("Should not be called")),
        cached_outputs: None,
    };

    let grad_input_cpu = CpuBackend::min_backward(&op_cpu, &grad_output_cpu)
        .map_err(|e| Error::InternalLogicError(format!("CPU min_backward failed: {}", e)))?;

    debug_print_tensor::<CpuBackend>(test_name, "CPU Min Backward Output", &grad_input_cpu);

    let grad_input_cuda = CudaBackend::min_backward(&op_cuda, &grad_output_cuda)
        .map_err(|e| Error::CudaError(format!("CUDA min_backward failed: {}", e)))?;

    debug_print_tensor::<CudaBackend>(test_name, "CUDA Min Backward Output", &grad_input_cuda);

    // Convert outputs to host for comparison
    let cuda_vec = CudaBackend::copy_to_host(&grad_input_cuda)
        .map_err(|e| Error::CudaError(format!("Failed to copy CUDA gradient to host: {}", e)))?;
    let cpu_vec = CpuBackend::copy_to_host(&grad_input_cpu).map_err(|e| {
        Error::InternalLogicError(format!("Failed to copy CPU gradient to host: {}", e))
    })?;

    assert_eq!(
        cuda_vec, cpu_vec,
        "Global min gradient mismatch: CPU={:?}, CUDA={:?}",
        cpu_vec, cuda_vec
    );
    println!("[{}] Global reduction gradient validated", test_name);

    // Test axis reduction backward
    println!("[{}] Testing axis reduction", test_name);
    let grad_output_cpu = CpuBackend::ones(&[3])?;
    let grad_output_cuda = CudaBackend::ones(&[3])?;
    let op_cpu = Op {
    ...
    let grad_input_cpu = CpuBackend::min_backward(&op_cpu, &grad_output_cpu)
        .map_err(|e| Error::InternalLogicError(format!("CPU min_backward axis failed: {}", e)))?;

    debug_print_tensor::<CpuBackend>(test_name, "CPU Min Backward Axis Output", &grad_input_cpu);

    let grad_input_cuda = CudaBackend::min_backward(&op_cuda, &grad_output_cuda)
        .map_err(|e| Error::CudaError(format!("CUDA min_backward axis failed: {}", e)))?;

    debug_print_tensor::<CudaBackend>(test_name, "CUDA Min Backward Axis Output", &grad_input_cuda);

    // Convert outputs to host for comparison
    let cuda_vec = CudaBackend::copy_to_host(&grad_input_cuda).map_err(|e| {
        Error::CudaError(format!("Failed to copy CUDA gradient axis to host: {}", e))
    })?;
    let cpu_vec = CpuBackend::copy_to_host(&grad_input_cpu).map_err(|e| {
        Error::InternalLogicError(format!("Failed to copy CPU gradient axis to host: {}", e))
    })?;

    assert_eq!(
        cuda_vec, cpu_vec,
        "Axis min gradient mismatch: CPU={:?}, CUDA={:?}",
        cpu_vec, cuda_vec
    );
    println!("[{}] Axis reduction gradient validated", test_name);
...
#[serial] // Add this attribute
#[test]
fn test_cuda_prod_backward() -> Result<(), Error> {
    let test_name = "test_cuda_prod_backward";
    let _guard = init_cuda_for_test(test_name)?;
    println!("[{}] CUDA context initialized", test_name);

    let data = vec![2.0, 3.0, 4.0];
    // Create Tensor objects
    let x_cpu_storage = CpuBackend::from_vec(data.clone(), &[3])?;
    let x_cuda_storage = CudaBackend::from_vec(data, &[3])?;
    let x_cpu = Tensor::<CpuBackend>::new(x_cpu_storage, false);
    let x_cuda = Tensor::<CudaBackend>::new(x_cuda_storage, false);
    ...
    println!("[{}] Computing CPU prod_backward", test_name);
    let grad_input_cpu = CpuBackend::prod_backward(&op_cpu, &grad_output_cpu)
        .map_err(|e| Error::InternalLogicError(format!("CPU prod_backward failed: {}", e)))?;

    debug_print_tensor::<CpuBackend>(test_name, "CPU Prod Backward Output", &grad_input_cpu);

    println!("[{}] Computing CUDA prod_backward", test_name);
    let grad_input_cuda = CudaBackend::prod_backward(&op_cuda, &grad_output_cuda)
        .map_err(|e| Error::CudaError(format!("CUDA prod_backward failed: {}", e)))?;

    debug_print_tensor::<CudaBackend>(test_name, "CUDA Prod Backward Output", &grad_input_cuda);
    ...
    assert_eq!(cuda_vec.len(), cpu_vec.len(), "Gradient length mismatch");
    ...
    println!("[{}] Gradient validation complete", test_name);
    Ok(())
}

#[serial] // Add this attribute
#[test]
fn test_cuda_logsumexp_backward() -> Result<(), Error> {
    let test_name = "test_cuda_logsumexp_backward";
    let _guard = init_cuda_for_test(test_name)?;
    println!("[{}] CUDA context initialized", test_name);
    ...
    println!("[{}] Computing CPU logsumexp_backward", test_name);
    let grad_input_cpu = CpuBackend::logsumexp_backward(&op_cpu, &grad_output_cpu)
        .map_err(|e| Error::InternalLogicError(format!("CPU logsumexp_backward failed: {}", e)))?;

    debug_print_tensor::<CpuBackend>(test_name, "CPU LogSumExp Backward Output", &grad_input_cpu);

    println!("[{}] Computing CUDA logsumexp_backward", test_name);
    let grad_input_cuda = CudaBackend::logsumexp_backward(&op_cuda, &grad_output_cuda)
        .map_err(|e| Error::CudaError(format!("CUDA logsumexp_backward failed: {}", e)))?;

    debug_print_tensor::<CudaBackend>(
        test_name,
        "CUDA LogSumExp Backward Output",
        &grad_input_cuda,
    );
